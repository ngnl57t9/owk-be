...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withSession; query != nil {
		...
		query.Where(predicate.Session(func(s *sql.Selector) {
			s.Where(sql.InValues(user.SessionColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_session
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_session" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_session" returned %v for node %v`, *fk, n.ID)
			}
